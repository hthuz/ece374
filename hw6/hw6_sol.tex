...
Then there will be two cases. In first case if $A[n] + L \le 5$, we can choose to cut $A[n]$ or reserve it as a usable wood. In the second case, if $A[n] + L > 5$, we must cut $A[n]$.
...
TCL(i, L) = 
  \begin{cases}
	  A[i] & \mbox{if } i = 0 \mbox{ and } A[i] \le 5 \\
	  0 &  \mbox{if } i = 0 \mbox{ and } A[i] > 5 \\
      max
	  \begin{cases}
		A[i] + TCL( i - 1 , L + A[i]) \\
		TCL(i - 1, 0) 
	  \end{cases} & \mbox{if } L + A[i] \le 5 \\
      TCL(i - 1,0) & \mbox{if } L + A[i] > 5
  \end{cases}
\end{equation*}
