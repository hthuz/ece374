...
$\bullet$ \textbf{Answer:} Construct our with "$\epsilon$" transitions between two DFAs. We change the original q state to $(q,positive)$ or $(q,negative)$. positive means it's M in postive direction, negative means it's another M in reverse direction. 
	...
	These two figures just show the general ideas. The number of accepting states can be any. Set NFA $N_1=(\sum,Q_1,\delta_1,s_1,A_1)$,which accepts the language $ThereAndBack(L)$, so we can get:
