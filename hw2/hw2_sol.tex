...
Set $L_1=\left\{ w \ \vline\ w \notin L(M) \right\}$ and $L_2=\left\{w \ \vline\ \overline{w} \in L(M)\right\}$ and $L_3=\left\{w \ \vline\ 1^{|w|} \in L(M)\right\}$.
	...
	And for $L3(M)$, We can consider the original DFA of L (M), and now we only need to add a new reject state $q_1$ and connect all the "0 arrowhead" to the state q1. As for q1 state, "0 or 1 arrowhead" will point to itself. To express it formally, we can get:
	
	$\bullet$ $Q_3 = Q \cup Q_1$, where $Q_1=\{q_1\}$

	$\bullet$ $\delta_3(q,1) = \delta(q,1)$, $\delta_3(q,0) = q_1$

	$\bullet$ $s_3 = s$

	$\bullet$ $A_3 = A$\\
	
	Finally, we can get $L' = L_1 \cap L_2 \cap L_3$ and use Q6.A to describe L' and so that we can describe the DFA and $n= \vline\ Q_1 \times Q_2 \times Q_3 \vline\ =  \lvert Q \rvert^2 (\lvert Q \rvert+1)= \lvert Q \rvert^3 + \lvert Q \ \rvert^2 $
